...
namespace KK_Plugins.MaterialEditor
{
    ...
    public class MaterialEditorCharaController : CharaCustomFunctionController
    {
        ...
        private IEnumerator RefreshClothesMainTexCoroutine()
        {
            ...
            for (var i = 0; i < MaterialTexturePropertyList.Count; i++)
            {
                ...
                if (property.TexID != null)
                    SetTextureWithProperty(FindGameObject(ObjectType.Clothing, property.Slot), property);
            }
        }
        ...
        /// <summary>
        /// Purge unused textures from the TextureDictionary
        /// </summary>
        private void PurgeUnusedTexture()
        {
            HashSet<int> unusedTexIDMap = new HashSet<int>(TextureDictionary.Keys);

            foreach (var prop in MaterialTexturePropertyList)
            {
                if (prop.TexID.HasValue)
                    unusedTexIDMap.Remove(prop.TexID.Value);

                if (prop.TexAnimationDef != null)
                    foreach (var frame in prop.TexAnimationDef.frames)
                        unusedTexIDMap.Remove(frame.texID);
            }

            foreach (var texID in unusedTexIDMap)
            {
